...
<div id="main-wrapper" class="<?php echo implode(' ', apply_filters( 'tc_main_wrapper_classes' , array('container') ) ) ?>">
    ...
    <center>
                            ...
                            if(!$_POST["combo"] and !$_POST["btn"]){
                              echo "  <H4> Please select the Lab subject</H4>";
                            }
                            else {
                                echo "<H4> Selected Lab subject is ".$_POST["combo"]."</H4>";
...
try{
    $query = "SELECT * FROM olLabSubTable";
    $result =mysqli_query($conn,$query);

    $storeArray = Array();
    while($row = mysqli_fetch_assoc($result)){
        $storeArray[] =  $row["labSubName"];
    }
}
catch(Exception $e){
...
<?php
    ...
    if($_POST["combo"] and !$_POST["btn"]){
        ...
        $query = "SELECT * FROM olLabTable where labAssignSub = '$sub'";
        $result = mysqli_query($conn,$query);
        $storeArray = Array();
        while($row = mysqli_fetch_assoc($result)){
            $storeArray[] =  $row["labAssignName"];
        }
